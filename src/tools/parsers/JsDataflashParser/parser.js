...
class DataflashParser {
  ...
  // ✅ Check if FMTU and its properties exist
  if (!FMTU || !FMTU.FmtType || !FMTU.UnitIds || !FMTU.MultIds) {
    console.warn("FMTU is incomplete or undefined:", FMTU)
    return
  }
  ...
  for (const index in FMTU.FmtType) {
    ...
    // Ensure this.FMT[type] is defined before assigning
    if (!this.FMT[type]) {
      this.FMT[type] = {}
    }
